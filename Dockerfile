...
WORKDIR /src
COPY ["PMS.PatientAPI.csproj", "."]
RUN dotnet restore "./PMS.PatientAPI.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "PMS.PatientAPI.csproj" -c Release -o /app/build

FROM build AS publish
RUN dotnet publish "PMS.PatientAPI.csproj" -c Release -o /app/publish

FROM base AS final
WORKDIR /app
COPY --from=publish /app/publish .
ENTRYPOINT ["dotnet", "PMS.PatientAPI.dll"]